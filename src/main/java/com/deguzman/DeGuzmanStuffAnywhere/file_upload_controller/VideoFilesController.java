...
public class VideoFilesController {
	...
	public ResponseEntity<List<PhotosFileInfo>> getListFiles() {
		List<PhotosFileInfo> photoFileInfos = (List<PhotosFileInfo>) videoFilesStorageService.loadAllVideos()
				.map(path -> {
					String filename = path.getFileName().toString();
					String url = MvcUriComponentsBuilder
							.fromMethodName(VideoFilesController.class, "getVideoFile", path.getFileName().toString())
							.build().toString();
